...
def init(W_indices, gitr_rz="assets/gitr_rz.txt"):
    ...
    r_inner_target = profiles.variables["r_inner_target"][:][W_indices_coords]
    z_inner_target = profiles.variables["z_inner_target"][:][W_indices_coords]
    ...
    if tile_shift_indices != []:
        for i, v in enumerate(tile_shift_indices):
            if i == 0:
                ...
                plt.axvline(x=rmrsCoords[v], color="k", linestyle="dashed")
    if Bangle_shift_indices != []:
        for i, v in enumerate(Bangle_shift_indices):
            if i == 0:
                plt.axvline(
                    x=rmrs[v], color="k", linestyle="dotted", label="$\Delta\\alpha_B$"
                )
            else:
                plt.axvline(x=rmrs[v], color="k", linestyle="dotted")

    plt.plot(rmrs, Bangle)
    plt.xlabel("D-Dsep [m]")
    plt.ylabel("Angle [$\circ$]")
    plt.title("Magnetic Pitch Angle, $\Delta\\alpha_B$")
    plt.show(block=False)
...
def ionization_analysis_theory(dt, W_surf):
    ...
    gridTemperature_Ionization = (
        ADAS.variables["gridTemperature_Ionization"][:] * 10
    )  # assuming in 0.1 eV
    IonizationRateGrid = np.abs(ADAS.variables["IonizationRateCoeff"][:][0, :, 0])

    IonizeRateInterpolated = np.interp(
        te, gridTemperature_Ionization, IonizationRateGrid
    )
...
def compare_surface_profiles(W_surf, varString):
    ...
    if len(varDims) == 1:
        ...
        varSOLPS = np.sum(profiles.variables[varString + "_inner_target"], axis=0)[
            W_surf
        ]
        print("length of SOLPS var:", len(rmrs))
